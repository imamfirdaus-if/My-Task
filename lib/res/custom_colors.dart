import 'package:flutter/material.dart';

class CustomColors {
  static final Color firebaseNavy = Color(0xFF2C384A);
  static final Color firebaseOrange = Color(0xFFF57C00);
  static final Color firebaseAmber = Color(0xFFFFA000);
  static final Color firebaseYellow = Color(0xFFFFCA28);
  static final Color firebaseGrey = Color(0xFFFFFFFF);
  static final Color googleBackground = Color(0xFF00AFC1);
  static final Color mytask_beranda = Color(0xFF40E0D0);
  static final Color mytask_key = Color(0xFF1E90FF);
  static final Color button = Color(0xFF006778);
}
